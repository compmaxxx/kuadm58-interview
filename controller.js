app.controller('barcodeController', function($scope, infoService) {
	...
	function handle_state_abnormal(info) {
		...
		if (info.result == 'not-found') {
			...
			$scope.notification_msg = 'ไม่มีข้อมูลผู้สมัคร'
			...
			if (parseInt(info.state) == 3 && helper.boolToInt(info.is_passed) == 0) {
				...
				$scope.notification_msg =
					'ผู้สมัครไม่ผ่านการสอบสัมภาษณ์(3) กลับบ้านไปซะ'
			} else if (parseInt(info.state) == 2 && (helper.boolToInt(info.is_study_plan_verified) ==
					0 || helper.boolToInt(info.is_gpa_verified) == 0)) {
				// Get out to home

				$scope.showname = true
				$scope.notification_msg = 'ผู้สมัครมีคุณสมบัติไม่ครบ(2) กลับบ้านไปซะ'

			}
			// else if(parseInt(info.state) == 1 && helper.boolToInt(info.documents) == 0){
			//   // Incorrect station and
			// }
			else {
				// Incorrect station
				$scope.showname = true
				$scope.notification_msg = 'มาผิดสถานีแล้วจ้ะ ไปสถานีที่ ' + (info.state +
					1)
